...
- **Authority:** the captain's interview answers of 2026-09-09, quoted below where they decide something
...
| Q5 store | the store is its own single-writer service holding the notes volume, with the Git helper and the Sync client as sibling containers in the same pod; the API is stateless and may run more than one replica; Admin is a route group in the API | "seperate the store" |
...
Six first-party images (api, store, git, obsidian-sync, curator, indexer)
plus PostgreSQL. The store's contract is a typed Protocol with an HTTP
