...
from coppermind_store.projections import write_projection
...
def test_a_later_revision_replaces_the_projection_it_finds(tmp_path: Path):
    relative, created = write(tmp_path)
    assert created is True

    replaced, created_again = write(tmp_path, revision=2, text="second", relative_path=relative)

    assert (replaced, created_again) == (relative, False)
