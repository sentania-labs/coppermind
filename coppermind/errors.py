...
# parser error quotes the note's own frontmatter. The internal surface keeps
# the cause, because `HttpStoreClient` reads it back to rebuild the typed
# error. Public is the default so a new caller cannot leak by forgetting to
# say.
